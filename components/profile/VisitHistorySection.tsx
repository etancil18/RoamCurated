...
export default function VisitHistorySection({
  ...
  const [selectedSticker, setSelectedSticker] =
    useState<StickerSelection | null>(null)

  const [stickerRouteLine, setStickerRouteLine] = useState<
    RouteLinePoint[]
  >([])
  ...
  const loadVisitHistory = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          payload?.error || 'Failed to load visit history.'
        )
      }
      ...
      setTotalVisits(
        typeof payload?.totalVisits === 'number'
          ? payload.totalVisits
          : nextCities.reduce(
              (sum, city) => sum + city.visitCount,
              0
            )
      )

      setExpandedCities((current) => {
        if (current.size > 0 || nextCities.length === 0) {
          return current
        }

        return new Set([nextCities[0].city])
      })
    } catch (err) {
      console.error(
  ...
  useEffect(() => {
    if (initialCities) {
      if (initialCities.length > 0) {
        setExpandedCities(new Set([initialCities[0].city]))
      }
  ...
  }, [initialCities, loadVisitHistory])
  ...
  const openDaySticker = async ({
    ...
    const eligibleVisits = day.visits
    ...
    const stops: StickerStop[] = eligibleVisits.map(
      (visit, index) => ({
        id: visit.id,
  ...
  const summary = useMemo(() => {
    if (totalVisits === 0) return 'No recorded visits yet.'

    const visitLabel = totalVisits === 1 ? 'place' : 'places'
    const cityLabel = totalCities === 1 ? 'city' : 'cities'

    return `${totalVisits} ${visitLabel} across ${totalCities} ${cityLabel}.`
  }, [totalCities, totalVisits])
        ...
        {loading ? (
          ...
          <div className="mt-5 space-y-3">
            {cities.map((cityGroup) => {
              const expanded = expandedCities.has(cityGroup.city)
