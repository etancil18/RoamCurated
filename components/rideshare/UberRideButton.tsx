...
export default function UberRideButton({
  ...
  const handleClick = () => {
    try {
      ...
      if (metadata.route_started_signal) {
        void Promise.resolve(
          logEvent("outing_route_started", {
            metadata: {
              planned_outing_id: plannedOutingId,
              event_id: eventId,
              destination_venue_id: toVenueId,
              destination_name: dropoff.name ?? null,
              signal_source: "uber_ride_to_first_stop",
            },
          })
        )
      }
    } catch (error) {
      console.warn("Failed to log outing Uber click", error)
    }
  }
  ...
  return (
    <a
      ...
      onClick={handleClick}
      aria-label={`Open Uber route to ${dropoff.name ?? "destination"}`}
      className={[
