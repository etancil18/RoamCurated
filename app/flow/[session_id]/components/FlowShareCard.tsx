...
export default function FlowShareCard({
    ...
    async function loadMapboxRouteLine() {
      ...
      try {
        ...
        for (let i = 1; i < validStops.length; i++) {
          ...
          if (!res.ok) {
            throw new Error(data?.error || 'Failed to build routed snapshot line')
          }
  ...
  const effectiveRouteLine =
    mapboxRouteLine.length >= 2 ? mapboxRouteLine : routeLine
  ...
  return (
    ...
    >
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,_rgba(99,102,241,0.32),_transparent_36%),radial-gradient(circle_at_bottom_right,_rgba(34,197,94,0.24),_transparent_38%)]" />
      ...
      >
        <div>
          <div className="flex items-center justify-between">
            ...
            <p className="rounded-full border border-white/15 bg-white/10 px-6 py-3 text-2xl font-semibold uppercase tracking-[0.2em] text-indigo-200">
              {humanizeStatus(status)}
            </p>
          </div>

          <div className="mt-28">
            <p className="text-3xl font-semibold uppercase tracking-[0.28em] text-indigo-300">
              {city ? city.toUpperCase() : 'Flow'}
            </p>
        ...
        <div className="rounded-[3rem] border border-white/10 bg-black/35 p-10 shadow-2xl backdrop-blur">
          <div className="mb-8 flex items-center justify-between">
            <p className="text-3xl font-bold">Route Snapshot</p>

            <p className="rounded-full bg-indigo-500/20 px-5 py-2 text-2xl font-semibold text-indigo-200">
          ...
          {plottedRoute.routePoints.length >= 2 ? (
            <div className="mb-10 overflow-hidden rounded-[2rem] border border-white/10 bg-neutral-950/70 p-4">
              <svg
                ...
                role="img"
                aria-label="Route line connecting completed Roam flow stops"
              >
                <defs>
                  <linearGradient id="flowRouteLineGradient" x1="0" x2="1" y1="0" y2="1">
                    <stop offset="0%" stopColor="#818cf8" />
                    <stop offset="100%" stopColor="#22c55e" />
                  </linearGradient>

                  <filter id="flowRouteGlow">
                ...
                <polyline
                  points={plottedRoute.routeLinePoints
                    ...
                    .join(' ')}
                  fill="none"
                  stroke="url(#flowRouteLineGradient)"
                  strokeWidth="7"
                  strokeLinecap="round"
            ...
            {displayStops.length > 0 ? (
              displayStops.map((stop, index) => (
                <div key={stop.id} className="flex gap-6">
                  <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-full bg-indigo-500 text-3xl font-black text-white">
                    {index + 1}
                  </div>
              ...
              <p className="text-3xl text-neutral-300">
                This Roam flow is ready to share.
              </p>
            )}
          </div>
        ...
        <div className="flex items-end justify-between gap-8">
          <div>
            ...
            <p className="mt-2 text-3xl font-bold text-white">
              Turning city intent into movement
            </p>
          </div>
...
function hasValidStopCoordinate(stop: FlowShareStop): boolean {
  return (
...
function buildRouteLabel(stops: FlowShareStop[]): string | null {
...
function buildSummary({
  ...
  if (status === 'completed') {
    return `Completed ${checkedInCount} of ${totalStops} stops on a Roam Flow.`
  }

  if (status === 'cancelled' || status === 'partial') {
    return `Checked in to ${checkedInCount} of ${totalStops} stops before ending this Roam Flow.`
  }

  return `Currently roaming through ${checkedInCount} of ${totalStops} stops.`
}
